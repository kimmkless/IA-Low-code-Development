...
def register_api_routes(app):
    ...
    def get_db_summary():
        """数据库概要：路径、设备数、数据条数、时间范围"""
        try:
            if not db_instance:
                return jsonify({'error': '数据库未初始化'}), 500
            summary = db_instance.get_database_summary()
    ...
    def list_sensors_api():
        """传感器列表：device_id、工作状态 work_status、是否在线 online 等（无需鉴权，便于调试）"""
        try:
            if not db_instance:
                return jsonify({'error': '数据库未初始化'}), 500
            try:
                minutes = int(request.args.get('online_within', 15))
            except ValueError:
                minutes = 15
            minutes = max(1, min(minutes, 24 * 60))
            sensors = db_instance.list_sensors(online_within_minutes=minutes)
            return jsonify({
                'sensors': sensors,
                'count': len(sensors),
                'online_within_minutes': minutes,
            })
        except Exception as e:
            logger.error(f"传感器列表失败: {e}")
            return jsonify({'error': str(e)}), 500

    @app.route('/api/sensors/<device_id>', methods=['GET'])
    def get_sensor_api(device_id):
        """单个传感器详情（含工作状态）"""
        try:
            if not db_instance:
                return jsonify({'error': '数据库未初始化'}), 500
            try:
                ...
                minutes = 15
            minutes = max(1, min(minutes, 24 * 60))
            sensor = db_instance.get_sensor_by_id(device_id, online_within_minutes=minutes)
            if not sensor:
                return jsonify({'error': '传感器不存在', 'device_id': device_id}), 404
            return jsonify(sensor)
        except Exception as e:
            logger.error(f"传感器详情失败: {e}")
            return jsonify({'error': str(e)}), 500
