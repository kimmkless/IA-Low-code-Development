...
function syncNodeVariableReferences(validVariableIds) {
    for (const node of state.nodes.values()) {
        ...
        if (node.type === 'get_sensor_info' || node.type === 'db_query' || node.type === 'environment_model' || node.type === 'analytics_summary' || node.type === 'advanced_prediction' || node.type === 'media_scene_model') {
